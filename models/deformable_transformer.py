...
class DeformableTransformer(nn.Module):
    def __init__(self, d_model=256, nhead=8,
        ...
        if args.cam_all == True:
            for i in range(num_encoder_layers):
                encoder_layers.append(
                    DeformableTransformerEncoderLayer(d_model, dim_feedforward,
                                                    dropout, activation,
                                                    num_feature_levels, nhead, enc_n_points, QSAttn=True)
                )
        else:
            for i in range(num_encoder_layers):
                encoder_layers.append(
    ...
    def forward(self, srcs, masks, pos_embeds, query_embed, category_codes, tsp, supp_class_id):
        ...
        # ********************  Encoder  ********************
        memory = self.encoder(
            src_flatten, spatial_shapes, level_start_index, valid_ratios, lvl_pos_embed_flatten, mask_flatten, category_codes, tsp, supp_class_id
        )
    ...
    def forward_supp_branch(self, srcs, masks, pos_embeds, query_embed, tsp, support_boxes, supp_class_id):
        assert query_embed is not None
        ...
        inter_category_codes_list = self.encoder.forward_supp_branch(
            src_flatten, spatial_shapes, level_start_index, valid_ratios, lvl_pos_embed_flatten, mask_flatten, tsp, support_boxes, supp_class_id
        )
...
class DeformableTransformerEncoderLayer(nn.Module):
    def __init__(self,
        ...
        # 残差连接卷积
        # self.forward_conv = nn.Conv1d(2*d_model, d_model, kernel_size=1, bias=False)
    ...
    def forward(self, src, pos, reference_points, spatial_shapes, level_start_index, padding_mask, category_codes, tsp, supp_class_id):
        # self attention
        ...
        if self.QSAttn:
            ...
            ##########################################################
            # siamese attention
            src, tsp = self.siamese_attn(src, inverse_sigmoid(category_codes), category_codes, tsp, supp_class_id)
            """
            q是待检测图像，k和v是支持类特征，最后相当于用支持类特征替代了待检测图像
            有些忽略的待检测图像本身的特征，这会造成一定的特征丢失
            """
            tmp_residual = residual.permute(0,2,1)
            tmp_src = src.permute(0,2,1)
            src = self.forward_conv(torch.cat([tmp_src, tmp_residual],dim=1))
    ...
    def forward_supp_branch(self, src, pos, reference_points, spatial_shapes, level_start_index, padding_mask, tsp, support_boxes, supp_class_id):
        # self attention
        # layer0需要经过siamese_attn
        # 在计算support特征编码时，不同类别之间的信息会有交互，因为此时class类不再是batch维度
        # 下面有category_code.unsqueeze(0).expand(src.shape[0], -1, -1)
        # (5,256) -> (5,5,256)
        src2 = self.self_attn(self.with_pos_embed(src, pos), reference_points, src, spatial_shapes, level_start_index, padding_mask)
        src = src + self.dropout1(src2)
        src = self.norm1(src)
...
class DeformableTransformerEncoder(nn.Module):
    ...
    def forward(self, src, spatial_shapes, level_start_index, valid_ratios, pos, padding_mask, category_codes, tsp, supp_class_id):
        output = src
        reference_points = self.get_reference_points(spatial_shapes, valid_ratios, device=src.device)
        for i, layer in enumerate(self.layers):
            output = layer(output, pos, reference_points, spatial_shapes, level_start_index, padding_mask, category_codes[i], tsp, supp_class_id)
        return output

    def forward_supp_branch(self, src, spatial_shapes, level_start_index, valid_ratios, pos, padding_mask, tsp, support_boxes, supp_class_id):
        output = src
        reference_points = self.get_reference_points(spatial_shapes, valid_ratios, device=src.device)
        category_codes = []
        for i, layer in enumerate(self.layers):
            output, category_code = layer.forward_supp_branch(
                output, pos, reference_points, spatial_shapes, level_start_index, padding_mask, tsp, support_boxes, supp_class_id
            )
